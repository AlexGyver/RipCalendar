...
virtual void fastSet(int x, int y, uint8_t fill = 1);                   // точка
...
```cpp
// пример наследования в класс. Нужно реализовать методы fastSet и update
class MAX7219 : public GyverGFX {
public:
  MAX7219(int width, int height) : GyverGFX(width * 8, height * 8) {}
  void fastSet(int x, int y, uint8_t fill = GFX_FILL) {
    // ...
  }
  void update() {
    // ...
